...
class Game1 {
    ...
    getStatus() {
        if (this.state === "lose") settingMessage("message-lose");
    ...
    moveEngine() {
        send(this.field);
        if (checkEmptyCell(this.field) === false && checkInline(this.field, "right") === false && checkInline(this.field, "left") === false && checkUpDown(this.field, "up") === false && checkUpDown(this.field, "down") === false) this.state = "lose";
        this.getStatus();
    }
}
...
document.addEventListener("keyup", (e)=>{
    if (currentGameData.state !== "playing") return false;
    if (tagRule.classList.contains("hidden")) return false;
    switch(e.code){
        case "ArrowRight":
...
function settingMessage(message) {
    ...
    for (const p of messageContainer)if (p.classList.contains(message)) {
...
function creativeGameField() {
    const rows = [
        ...field.querySelectorAll("tr")
    ];
    const cells = rows[0].children.length;
    const gameField = [];
    for(let i = 0; i < rows.length; i++)gameField.push(Array(cells).fill(0));
    return gameField;
}
function getRandomCoord() {
    ...
    function getNubmerOfRows() {
        const rowNubmer = 0 + Math.random() * 4;
...
function creativeElement(obj, state) {
    if (checkEmptyCell(obj)) {
        ...
        if (state === "wait") randomDigit = 2; // set 1024 to check win
        if (row[colRow[1]] === 0) row[colRow[1]] = randomDigit;
        else creativeElement(obj);
        return obj;
    } else settingMessage("message-lose");
    return obj;
}
    ...
    for(let i = 0; i < obj.length; i++){
        ...
        for(let n = 0; n < row.length; n++){
            ...
            if (row[n] === 0) {
                ...
                if (row[n] === goal) settingMessage("message-win");
            }
        }
    }
}
function toLeftRight(obj, direction) {
    for(let i = 0; i < obj.length; i++)obj[i] = move(obj[i], direction);
    return obj;
}
...
function move(data, direction) {
    ...
    if (array2.length > 1) {
        for(let i = 0; i < array2.length - 1; i++)if (array2[i] === array2[i + 1]) {
            array2[i] = array2[i] * 2;
            array2[i + 1] = 0;
            score += array2[i];
        }
    ...
    while(array3.length < 4)switch(direction){
        case "right":
            array3.unshift(0);
            break;
        case "left":
...
function checkUpDown(obj, direction) {
    for(let i = 0; i < obj.length; i++){
        ...
        switch(direction){
            case "down":
                if (coloumn[index + 1] === 0) return true;
                for(let m = index; m < coloumn.length - 1; m++){
                    if (coloumn[m] === coloumn[m + 1]) return true;
                    if (coloumn[m + 1] === 0) return true;
                }
                break;
            case "up":
                if (coloumn[index - 1] === 0) return true;
